...
import {
  EventPaginationResponseMapper,
  EventResponseMapper,
} from '../response/event/event-response-mapper';
...
export class UserFindEventsUseCase {
  ...
  async findAll(
    ...
    const { items, total } = await this.eventsRepository.findAllMyEventsWithFiltersByOwnerId(
      userPersonId,
      filter,
    );
  ...
  async findOneEventByUserAndEventId(
    userPersonId: string,
    eventId: string,
