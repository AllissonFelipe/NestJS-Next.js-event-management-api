...
export class FindMyEventReportsUseCase {
  ...
  async execute(userPersonId: string, queryDto: MyEventReportsQueryDto): Promise<MyEventsReportsWithQueryResponseDto> {
    ...
    return MyEventsReportsWithQueryResponseMapper.toResponse(items, pagination.page, pagination.limit, total);
  }
  ...
  async executeFindOne(userPersonId: string, eventId: string): Promise<MyEventReportResponseDto> {
    ...
    const eventReport = await this.eventReportRepository.findByPersonIdAndEventId(userPerson.id, event.id);
    if (!eventReport) {
      throw new EventReportNotFoundError();
    }
    return MyEventReportResponseMapper.toResponse(eventReport, eventReport.event, eventReport.reporter);
  }
}
